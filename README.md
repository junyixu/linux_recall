...
You can jump back to the source with one key: the web page, the PDF page in Zotero, or the Anki card.
...
- **Search**: `jq` recipes and `lr`, an fzf picker with the matching line boxed in red on the screenshot (rendered inline in kitty), and `ctrl-o` to reopen the URL / PDF page / Anki card
...
flowchart LR
    ...
    B & Z & A & C --> D{"daemon only:<br/>same app and dHash ≥ 0.95?"}
    D -->|yes| X["drop"]
...
lr lunar                   # fzf: app │ matching line, keyword in red │ time + site / paper / deck
                           #   preview: screenshot with the match boxed (kitty), enter: open image,
                           #   ctrl-o: reopen the URL / Zotero page / Anki card
