...
- deb 包：推送 `v*` tag（必须和 `pyproject.toml` 的 `version` 一致）后，`.github/workflows/deb.yml` 跑 `scripts/build-deb.sh` 并建 GitHub Release。Debian 没有 `onnxruntime` / `rapidocr`，所以包里自带 uv 管理的 Python 和按 `uv.lock` 装的 venv（`/usr/lib/linux-recall`），RapidOCR 模型在构建时下载好。venv 里是绝对路径，脚本要 sudo 直接在 `/usr/lib/linux-recall` 构建，不要在本机跑；加了文件（脚本、unit 等）要同时改 `build-deb.sh` 和 AUR 的 PKGBUILD
...
- `apps/inkycap.py`：InkyCap（Tauri 写的 Typst 笔记软件）没有 CLI / IPC，窗口标题永远是 `InkyCap`（KWin 的 `resource_class` 是 `inkycap`）。前端每次切到一篇笔记，约 0.5 s 后把它的绝对路径写进 `<notebox>/.inkycap/local.json` 的 `last_active_file`；`~/.config/inkycap/config.json` 列出所有 notebox，每个窗口一个 notebox，所以取 `local.json` 最近修改的那个（多个时 `ambiguous`）。title / date / zid / tags / aliases 直接解析笔记开头的 `#note(...)`，不跑 `typst query`（约 0.8 s CPU）。拿不到滚动位置，所以没有 `lines`；当前标签页不是笔记（collection、mycelial、空标签页）时仍是上一篇。没有能让 InkyCap 打开某个文件的接口，所以没有 `open_link`；`lrf` 的 `ctrl-t` 输出 `path`。测试：`uv run python -m linux_recall.apps.inkycap`
- `apps/okular.py`：Okular 自己的 D-Bus 接口。每个进程是 `org.kde.okular-<pid>`（KWin 给的 pid），每个文档标签页是 `/okular`、`/okular2`…（进程内计数，关掉的标签页留空号，所以从 `/` 的 Introspect 列出来），有 `currentDocument`（本地路径）、`currentPage`（从 1 开始，没文档时 0）、`pages`、`documentMetaData(title/author/mimeType)`。不标记活动标签页，所以按窗口标题 `<标题> — Okular` 匹配：标题是文档 title（有且设置里开了“标题栏显示文档标题”）、文件名或完整路径；改过的文档标题后面有 ` *`。只有一个标签页却对不上时 `match: "only"`。约 2 ms。`lrf` 的 `ctrl-o` 跑 `okular --page N 文件`
...
- 顶层字段：`schema_version`（当前是 13）、`id`、`captured_at`（带时区的 ISO 8601）、`trigger`（`hotkey` / `timer` / 测试时自定义）、`screenshot`、`screens`、`window`、`browser`、`ocr`
...
6. 更多应用上下文：✅ Anki（AnkiConnect）、Obsidian（CLI）、kitty（shell 命令和输出、Neovim、Claude Code）、Neovide、Okular（D-Bus）；待做：Dolphin
