...
- `claude.py`：`~/.claude/sessions/<pid>.json`（session id、cwd、状态）+ 会话记录 `~/.claude/projects/*/<session id>.jsonl` 末尾 512 KB 里最新的 `ai-title` 和 `last-prompt`（会话记录可以有几十 MB，不要整个读）
